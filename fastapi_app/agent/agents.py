...
class MedicalAnalyticsAgent:
    ...
    def execute_query(self, query: str, max_iterations: int = 10) -> ExecuteResponse:
        ...
        for iteration in range(max_iterations):
            ...
            # Собираем метрики использования токенов из response_metadata
        ...
        # Собираем метрики из финального вызова
        ...
        logger.info(
            f"Execute completed: {len(summary_text)} chars, {len(charts)} charts, "
            f"tokens: {total_input_tokens}/{total_output_tokens}, latency: {latency_ms:.2f}ms, cost: ${total_cost:.6f}"
        )
